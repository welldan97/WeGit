...
const WeWeWeChatApp = require('./WeWeWeChatApp');
...
module.exports = class WgOs {
  ...
  _onMessage({ message }) {
    ...
    switch (type) {
      case 'os:userSave':
        return this._onOsUserSave({ message });
      case 'os:createApp':
        return this._onCreateApp({ message });
  ...
  _onCreateApp({ message }) {
    this._saveApps([message.payload]);
  }
  ...
  _onOsUserSave({ message }) {
    const { payload } = message;

    this._saveUser(payload.user);
    this._saveApps(payload.apps);
  }
};
