  ...
  class WgMesh {
    ...
    _onMessage(event) {
      ...
      const wgConnection = this.wgConnections.find(
        c => c.eventTarget === event.target,
      );

      const nextData = { ...event.data, sender: wgConnection.receiver };
      ...
      if (receiver !== this.sender)
        return this._onPassMessage(wgConnection, event.data);

      switch (type) {
        case 'mesh:handshake':
          return this._onHandshake(wgConnection, event.data);
        case 'mesh:offer':
          return this._onOffer(wgConnection, event.data);
        case 'mesh:answer':
          return this._onAnswer(wgConnection, event.data);
        default:
      }
...
*/
    _addConnection(wgConnection) {
      ...
      wgConnection.eventTarget.addEventListener('message', e =>
        this._onMessage(e),
      );
      wgConnection.eventTarget.addEventListener('change', e =>
        this._onChange(e),
      );
