...
module.exports = ({ fs, git, gitInternals, dir = '.' }) => {
  ...
  const containedOidsBetween = async ({ hasOid, wantOid }) => {
    ...
    const allOids = (
      await Promise.all(oidsBetween.map(o => getCommitContainedOids(o)))
    ).flat();
  ...
  const getCommitHolders = async (oid, depth) => {
    if (depth === 0) return [];
    //return [];
    const commitHolder = await git.readCommit({
      dir,
    ...
    const allCommits = await Promise.all(
      parent.map(async c => getCommitHolders(c, depth - 1)),
    );
    return [commitHolder, ...allCommits.flat()];
    //console.log(parent);
  };

  const listCommitHoldersImplementation = async depth => {
    const headCommitOid = await git.resolveRef({ dir, ref: 'HEAD' });

    const commits = await getCommitHolders(headCommitOid, depth);

    return commits
      .filter((c, i) => commits.findIndex(cc => cc.oid === c.oid) === i)
      .slice(0, 5);
  };
  ...
  const helpers = {
    ...
    async applyDiffBundle(diffBundle) {
      ...
      if (!hasHead && refDiff.length) {
        await git.writeRef({
          dir,
          ref: 'HEAD',
          value: refDiff.filter(r => r.ref !== 'HEAD')[0].ref,
          symbolic: true,
        });
        hasHead = true;
      }
    ...
    async listCommitHolders(depth) {
      return listCommitHoldersImplementation(depth);
    },
  };
  return helpers;
};
