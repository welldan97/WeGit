...
import { toWgKey, fromWgKey } from './wgKey';
...
export default class WgConnection extends EventTarget {
  [Symbol.toStringTag]() {}
  constructor({ id, sender, receiver } = {}) {
    ...
    this.connection.addEventListener('connectionstatechange', e => {
      if (e.target.connectionState === 'connected') return;
      this.dispatchEvent(new Event('change'));
    });
  }
  ...
  async establish(wgAnswerKey) {
    const { answer, candidates: inCandidates, sender } = fromWgKey(wgAnswerKey);
  ...
  send(object) {
    this.sendChannel.send(
      JSON.stringify({ path: [], ...object, version: VERSION }),
    );
  }
}
